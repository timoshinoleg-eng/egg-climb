...
try{
  ...
  const sample=page.evaluate(()=>new Promise(resolve=>{
    ...
    const frame=now=>{
      ...
      resolve({phase:document.body.dataset.phase,physicsTicks:Number(stage.dataset.tick)-initialTick,queued:Number(stage.dataset.queue),frames:times.length,durationMs:now-start,fps:times.length*1000/(now-start),p95Ms:sorted[Math.ceil(sorted.length*0.95)-1],maxMs:sorted.at(-1),overBudgetFrames:times.filter(ms=>ms>20).length,presentationFps:Number(stage.dataset.fps),clientRafFps:Number(stage.dataset.rafFps),renderWorkP95Ms:Number(stage.dataset.workP95),quality:stage.dataset.quality,dpr:Number(stage.dataset.dpr),renderStride:Number(stage.dataset.renderStride??1),profile:stage.dataset.profile,particles:stage.dataset.particles})
  ...
  const minPresentationFps=result.renderStride>1?26:58.5
  result.pass=result.fps>=58.5&&result.p95Ms<=20&&result.presentationFps>=minPresentationFps&&result.clientRafFps>0&&result.renderWorkP95Ms<8&&result.phase==='playing'&&result.physicsTicks>=330&&result.queued<=8&&(!kitchen||result.overview)&&errors.length===0
