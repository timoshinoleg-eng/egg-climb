...
function fakeWorker(t, mode='silent') {
  ...
  class FakeWorker {
    ...
    constructor(url){this.url=url;FakeWorker.last=this}
...
test('watchdog rejects a stalled worker and terminates all retained requests', async t => {
  const Fake=fakeWorker(t),host=new WorkerSimulationHost('https://example.invalid/sim-worker.js',undefined,undefined,undefined,20)
...
test('synchronous postMessage failure rejects immediately instead of leaking a pending request', async t => {
  const Fake=fakeWorker(t,'throw'),host=new WorkerSimulationHost('https://example.invalid/sim-worker.js',undefined,undefined,undefined,1000)
...
test('successful requests clear watchdogs and free coalesces concurrent callers', async t => {
  const Fake=fakeWorker(t,'auto'),host=new WorkerSimulationHost('https://example.invalid/sim-worker.js',undefined,undefined,undefined,20)
...
test('synchronous teardown rejects an in-flight init and cancels its timeout', async t => {
  const Fake=fakeWorker(t),host=new WorkerSimulationHost('https://example.invalid/sim-worker.js',undefined,undefined,undefined,1000)
...
test('level selection and the request watchdog coexist in the Worker constructor', async t => {
  const Fake = fakeWorker(t)
  const host = new WorkerSimulationHost('https://example.invalid/sim-worker.js', undefined, undefined, KITCHEN_LEVEL, 1000)
  const pending = host.init()
  assert.equal(new URL(Fake.last.url).searchParams.get('level'), KITCHEN_LEVEL.id)
  const message = Fake.last.messages[0]
  Fake.last.emit({
    id: message.id, protocolVersion: WORKER_PROTOCOL_VERSION, type: 'initialized', snapshot: { tick: 0 },
    runtimeInfo: { ...EXPECTED_WORKER_RUNTIME_INFO, levelId: KITCHEN_LEVEL.id, levelVersion: KITCHEN_LEVEL.version,
      levelFormatVersion: KITCHEN_LEVEL.formatVersion, levelHash: KITCHEN_LEVEL.hash },
  })
  try { assert.deepEqual(await pending, { tick: 0 }) } finally { host.terminate() }
  assert.equal(Fake.last.terminated, true)
})

test('watchdog hardening does not weaken the authoritative level handshake', async t => {
  const Fake = fakeWorker(t)
  const host = new WorkerSimulationHost('https://example.invalid/sim-worker.js', undefined, undefined, KITCHEN_LEVEL, 1000)
  const pending = host.init(), message = Fake.last.messages[0]
  Fake.last.emit({ id: message.id, protocolVersion: WORKER_PROTOCOL_VERSION, type: 'initialized',
    snapshot: { tick: 0 }, runtimeInfo: EXPECTED_WORKER_RUNTIME_INFO })
  await assert.rejects(pending, /handshake mismatch/)
  assert.equal(Fake.last.terminated, true)
})
