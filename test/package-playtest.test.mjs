...
test('static playtest package contains only browser runtime assets and preserves subpath imports', async () => {
  ...
  assert.match(root, /const maxLaunch=params\.get\('max'\)==='1'/)
  assert.match(root, /const allowMaxLab=params\.get\('lab'\)==='1'/)
  assert.match(root, /const lab=\['feel','physics','scenario','visual','order'\]/)
  assert.match(root, /if\(maxLaunch&&!allowMaxLab\)/)
  assert.match(root, /\.\/play\/kitchen\.html/)
  assert.match(root, /\.\/debug\/index\.html/)

  const max = await readFile(join(output, 'max.html'), 'utf8')
  assert.match(max, /params\.set\('max','1'\)/)
  assert.match(max, /\['feel','physics','scenario','visual','order','lab','mode'\]/)
  assert.match(max, /\.\/play\/kitchen\.html/)
  assert.doesNotMatch(max, /\.\/debug\/index\.html/)

  const html = await readFile(join(output, 'debug', 'index.html'), 'utf8')
  assert.match(html, /\.\.\/vendor\/three\/three\.module\.js/)
  assert.match(html, /const allowMaxLab = params\.get\('lab'\) === '1'/)
  assert.match(html, /params\.get\('max'\) === '1'/)
  assert.match(html, /params\.has\('WebAppStartParam'\)/)
  assert.match(html, /location\.hash\.includes\('WebAppData='\)/)
  assert.match(html, /if \(maxLaunch && !allowMaxLab\)/)
  assert.match(html, /location\.replace\('\.\.\/play\/kitchen\.html'/)
