...
test('packaged playtest runs under an /egg-climb/ subpath and exports replayable JSON', async ({ page, browserName }) => {
  ...
  try {
    ...
    await expect(page.locator('#gameStage')).toHaveAttribute('data-profile', 'performance-v2')
    await expect(page.locator('#gameStage')).toHaveAttribute('data-render-stride', '2')
    await expect(page.locator('#motionButton')).toBeDisabled()
    await expect(page.locator('#motionButton')).toHaveAttribute('aria-label', 'Reduced motion locked for performance')
    await expect(page.locator('#onboardingTitle')).toContainText('egg-sized escape')
    await page.locator('#startButton').click()
    await expect.poll(async () => Number(await page.locator('#gameStage').getAttribute('data-fps'))).toBeGreaterThan(5)
    const telemetry = await page.locator('#gameStage').evaluate(stage => ({ fps: Number(stage.dataset.fps), rafFps: Number(stage.dataset.rafFps) }))
    expect(telemetry.rafFps).toBeGreaterThan(telemetry.fps * 1.5)
    await expect(page.locator('#performance')).toContainText('Hz')

    await page.goto(`http://127.0.0.1:${port}/egg-climb/?max=1&feel=2d-hold-assist&scenario=jump-base`)
    await expect(page).toHaveURL(/\/egg-climb\/play\/kitchen\.html\?max=1$/)
    await expect(page.locator('#gameStage')).toHaveAttribute('data-level', 'kitchen-escape-v1')

    await page.goto(`http://127.0.0.1:${port}/egg-climb/debug/index.html?max=1&feel=2d-hold-assist&scenario=jump-base`)
    await expect(page).toHaveURL(/\/egg-climb\/play\/kitchen\.html\?max=1$/)
    await expect(page.locator('#gameStage')).toHaveAttribute('data-level', 'kitchen-escape-v1')
